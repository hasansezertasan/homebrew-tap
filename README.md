...
|---------|--------------|------|---------------|
| [`cobo`](https://github.com/hasansezertasan/cobo) | Generic CLI for fetching boilerplate files from configurable git repositories | Formula | Yes |
| [`hwid`](https://github.com/hasansezertasan/hwid) | Cross-platform hardware ID extraction using native OS detection | Formula | Yes |
| [`keycast`](https://github.com/hasansezertasan/keycast) | Cross-platform keystroke and mouse click visualizer built in Python | Formula + Cask | Formula: yes / Cask: no |
| [`micoo`](https://github.com/hasansezertasan/micoo) | Easy access to mise-cookbooks | Formula | Yes |
...
```sh
brew install hasansezertasan/tap/cobo
brew install hasansezertasan/tap/nur
brew install --cask hasansezertasan/tap/keycast
```

> **Note on `keycast`:** it ships as **both** a formula (the Python CLI, macOS
> only) and a cask (the prebuilt `.app`), so `brew install keycast` is
> ambiguous — disambiguate with `--formula` or `--cask` (see the note above).
